...
In the outer stage, this `do` block creates a point map indicating how 
...
is the game **balanced**, and is the game **fair**? In particular, we asked these research questions:

#### Is there a balance between player choice and the potential for strategy?

![Agram Branching Factor]({{site.url}}{{site.baseurl}}/images/agram/AgramBFRev.png){:class="post-image"}

The above image shows the average player decision branching factor in a four-player game 
...
player for each trick. We can see the effect of being forced to follow suit when possible. 
...
#### What is the smallest number of tricks that still allows for a fair game?
