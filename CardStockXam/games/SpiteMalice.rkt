   ...
   (stage player
     ...
     (do (
          ((== (size ((current player) iloc HAND)) 0) ;; if hand is empty, refill hand (only if last card not played to reserve!!)
           ...
           ((and (< (size ((current player) iloc HAND)) 'HANDSIZE) (< (size (game iloc STOCK)) 'HANDSIZE))
            (let (- (size (game iloc STOCK)) (- 'HANDSIZE (size ((current player) iloc HAND)))) 'SIZE
              (do (
                   ((< 'SIZE 0)
                    (repeat all
                            (move (top (game iloc STOCK)) (top ((current player) iloc HAND)))))
     ...
     (choice ( ;; hand to reserve
       (do (
        (any ((current player) iloc HAND) 'DISC
            (any 'PLOCS 'L
               (do (
                    (move 'DISC (top ((current player) vloc 'L)))
                           ;; if next player needs cards & stock >= 'HANDSIZE
