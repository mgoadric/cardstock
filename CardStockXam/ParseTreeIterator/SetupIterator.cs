		...
		public static GameActionCollection ProcessSetup(RecycleParser.SetupContext setupNode){
			...
			if (setupNode.deckcreate() != null){
				...
				foreach (var deckinit in decks) {				
					var locstorage = CardIterator.ProcessSubLocation(deckinit.locstorage().locpre(), deckinit.locstorage().locpost(),deckinit.locstorage().GetChild(2).GetText() == "iloc",deckinit.locstorage().GetChild(2).GetText() == "mem");
					var deckTree = DeckIterator.ProcessDeck(deckinit.deck());
					var repeat = 1;
					if (deckinit.deck().@int() != null){
						repeat = IntIterator.ProcessListInt(deckinit.deck().@int())[0];
					}
